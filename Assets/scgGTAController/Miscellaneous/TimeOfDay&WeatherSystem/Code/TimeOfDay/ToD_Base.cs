...
public class ToD_Base : MonoBehaviour 
{
    ...
    /// <summary>
    /// All lightpole objects in the scene
    /// </summary>
    private GameObject[] lightpoles;
    bool lightPolesActivated;
    ...
    void Start()
    {
        ...
        lightpoles = GameObject.FindGameObjectsWithTag("lightPoleLight");
    }
    ...
    void Update()
    {
        ...
        if (Get_fCurrentTimeOfDay > 0.74f && Get_fCurrentTimeOfDay < 0.76f && !lightPolesActivated)
        {
            print("turning on lights!");
            foreach (GameObject go in lightpoles)
            {
                if (go)
                {
                    go.GetComponent<Light>().enabled = true;
                    go.GetComponent<VolumetricLight>().enabled = true;
                }
            }

            // Set the flag to true to indicate that the code has been executed
            lightPolesActivated = true;
        }

        if (Get_fCurrentTimeOfDay > .24f && Get_fCurrentTimeOfDay < 0.26f && lightPolesActivated)
        {
            ...
            foreach (GameObject go in lightpoles)
            {
                if (go)
                {
                    ...
                    go.GetComponent<VolumetricLight>().enabled = false;
                }
            }

            // Reset the flag to false to allow the code to be executed again
            lightPolesActivated = false;
        }
    }
