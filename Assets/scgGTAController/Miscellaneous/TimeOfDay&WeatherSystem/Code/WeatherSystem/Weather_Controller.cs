...
public class Weather_Controller : MonoBehaviour 
{
    ...
    public Material matCloudsLow;
    public Material matCloudsHigh;
    public float cloudDefaultTransparencyLow;
    public float cloudDefaultTransparencyHigh;
    ...
    public void UpdateAllWeather(float sunIntensity, Color sunLightColor, float moonIntensity, Color moonLightColor, Color skyTint, Color skyGround, Color cloudColor, float fogDensity, Color fogColor, float fadeTime)
    {
        ...
        // Cloud settings
        if (matCloudsLow != null && matCloudsHigh != null)
        {
            //Set low clouds transparency
            Color cColor = matCloudsLow.GetColor("_CloudColor");
            
            float currentTimeOfDay = gTimeOfDay.GetComponent<ToD_Base>().Get_fCurrentTimeOfDay;
            
            float alpha = .015f;

            if (currentTimeOfDay > .5f)
            {
                alpha = Mathf.Lerp(cloudDefaultTransparencyLow, .015f, ((currentTimeOfDay - .5f) / .5f) * 2f);
            }
            else if (currentTimeOfDay > 0.20f)
            {
                // Calculate alpha for fading back in
                alpha = Mathf.Lerp(.015f, cloudDefaultTransparencyLow, ((currentTimeOfDay - 0.20f) / 0.30f));
            }

            // Set the alpha value to your color variable
            cColor.a = alpha;

            matCloudsLow.SetColor("_CloudColor", cColor);

            // Calculate the speed based on the day cycle duration
            float speed = -.25f * (60f / gTimeOfDay.GetComponent<ToD_Base>().GetSet_fSecondInAFullDay);

            // Set the speed parameter in the material
            matCloudsLow.SetFloat("_Speed", speed);

            Color cColor2 = matCloudsHigh.GetColor("_CloudColor");

            //Set high clouds transparency
            float alpha2 = .015f;

            if (currentTimeOfDay > .5f)
            {
                alpha2 = Mathf.Lerp(cloudDefaultTransparencyHigh, .015f, ((currentTimeOfDay - .5f) / .5f) * 2f);
            }
            else if (currentTimeOfDay > 0.20f)
            {
                // Calculate alpha for fading back in
                alpha2 = Mathf.Lerp(.015f, cloudDefaultTransparencyHigh, ((currentTimeOfDay - 0.20f) / 0.30f));
            }

            // Set the alpha value to your color variable
            cColor2.a = alpha2;

            matCloudsHigh.SetColor("_CloudColor", cColor2);

            // Calculate the speed based on the day cycle duration
            float speed2 = .25f * (60f / gTimeOfDay.GetComponent<ToD_Base>().GetSet_fSecondInAFullDay);

            // Set the speed parameter in the material
            matCloudsHigh.SetFloat("_Speed", speed2);
        }
        else
        {
